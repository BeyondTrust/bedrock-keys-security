...
credential surface and the `BedrockAPIKey-*` phantom user pattern.
...
|------|--------------|-------------|
| `1-block-all-keys.json` | Deny `iam:CreateServiceSpecificCredential` for `bedrock.amazonaws.com` and deny `bedrock:CallWithBearerToken` org-wide | Full lockout of Bedrock API keys in accounts that should never use them |
...
| `3-block-long-term-only.json` | Deny `bedrock:CallWithBearerToken` when `bedrock:BearerTokenType = LONG_TERM` | Allow short-term API key use while killing static ABSK keys |
