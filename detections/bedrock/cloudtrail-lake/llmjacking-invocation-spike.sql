-- LLMjacking invocation spike from a Bedrock API key principal.
--
-- Detects any Bedrock API key principal (long-term phantom user OR
-- short-term STS-derived API key) that issues more than 100 Bedrock
...
-- (not a struct), so the API key-token field is read with element_at and
